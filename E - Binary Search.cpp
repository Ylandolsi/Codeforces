...
public:
     ...
     void Test_Case(){
          read( n , k ) ; 
          int r = n+1, l =  1 ;
          vector<int>vect(n) , deletions ;
          for(auto &d : vect )  cin>>d;
          set<int>indexExclus , indexofelsmallerthanK ; 
          for ( int i = 0 ; i < n ; i++ ){
               indexExclus.insert(i);
               if ( vect[i] < k ) indexofelsmallerthanK.insert(i);
               
          }
          for ( int i = 0 ; i < n ; i++ ){
               if ( vect[i] == k ){ indexofK = i ;  break ;}
          }
          
          while ( r - l > 1  ){
               int mid = (l+r)/2 ;
               indexExclus.erase(mid-1);
               if ( vect[mid-1] < k ) deletions.pb(mid-1);
               if ( vect[mid-1] <= k ) l = mid ;
               else r = mid ; 
          }

          if  ( deletions.size() ){
               for ( int i = 0 ; i < deletions.size() -1  ; i++ )
                    indexofelsmallerthanK.erase(deletions[i]);
          ...
          if ( indexExclus.find(indexofK) != indexExclus.end()) 
               write ( 1 , "\n" , indexofK + 1  , " " , l  , "\n");
          
          else {
               int x = *indexofelsmallerthanK.begin() + 1 ; 
               write (  2 , "\n", x , " " , indexofK + 1  , "\n" , x  , " " , l  , "\n");
